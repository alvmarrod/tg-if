...
@pytest.fixture
async def reg_and_storage() -> AsyncGenerator[
    tuple[UploadRegistry, MagicMock, TemporaryDirectory[str]], Any
]:
...
class TestHandleUploadPost:
    ...
    async def test_successful_upload(
        ...
        entry = await reg.get_by_hash(_hash(payload))
    ...
    async def test_cache_hit_returns_file_id(
        ...
        await reg.update_file_id(content_hash, "AgAC_existing", "QQAD_existing")
