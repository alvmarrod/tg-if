...
| Layer | File | Status |
|-------|------|--------|
| Domain entities | `src/domain/entities.py` | Implemented (Pydantic models) |
| Domain schemas | `src/domain/schemas.py` | Empty stub |
| Domain rules | `src/domain/rules.py` | Empty stub |
| App event_dispatcher | `src/app/event_dispatcher.py` | Empty stub |
| App receiver_service | `src/app/receiver_service.py` | Empty stub |
| App response_consumer | `src/app/response_consumer.py` | Empty stub |
| Infra config | `src/infrastructure/config.py` | Implemented |
| Infra health | `src/infrastructure/health.py` | Implemented |
| Infra telegram/client.py | `src/infrastructure/telegram/client.py` | Empty stub |
| Infra telegram/handlers.py | `src/infrastructure/telegram/handlers.py` | Empty stub |
| Infra broker/* | `src/infrastructure/broker/*` | Empty stubs |
...
| Entrypoint | `main.py` | Implemented (async) |

**Only production code**: `src/domain/entities.py` (EventType, ChatType enums; RoutingContext, TelegramEvent, MessageEvent, CommandEvent, CallbackQueryEvent, OutgoingMessage models).
...
1. **Phase 1 — Foundation**: Domain data structures, config loading, basic logging (entities.py done, rest pending)
2. **Phase 2 — Vertical Slice**: Single bot receive -> publish to broker -> consume response -> send to Telegram
